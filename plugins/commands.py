...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('💕 ᴄʜᴀɴɴᴇʟ ✨', url='https://t.me/moviesss4ers'),
                    InlineKeyboardButton('💖 ɢʀᴏᴜᴘ 💞', url='https://t.me/moviei43')
                ],[
                    InlineKeyboardButton('✨ ʜᴇʟᴘ 🛐', callback_data='help'),
                    InlineKeyboardButton('🥲 ᴀʙᴏᴜᴛ ᴍᴇ ☹️', callback_data='about')
                ],[
                    InlineKeyboardButton('ᴀɴʏ ǫᴜᴇʀɪᴇs ᴅᴍ ʜᴇʀᴇ ✅',url='https://t.me/Link_Reporter_Bot')
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "🔥 ᴊᴏɪИ ᴜᎮᴅΛᴛΞs ᴄʜΛИИΞʟ 🔥", url=invite_link.invite_link
                )
            ]
        ]
...
async def shortlink(bot, message):
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(f"<b>Hey {message.from_user.mention}, This command only works on groups !\n\nIf you want to connect your shortner & api with this bot in your groups then contact @Nexus_Shubhu.</b>")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
